...
function GunlukTakipPage({ kayitlar, addGunlukKayit, deleteGunlukKayit }) {
  ...
  const toplamKalori = kayitlar.reduce(
    (sum, item) => sum + Number(item.kalori),
    0
  );
    ...
    <div className="page" style={{ animation: "fadeIn 0.5s ease-in-out" }}>
      ...
      <div className="card" style={{ padding: "25px", borderRadius: "20px", backgroundColor: "white", marginBottom: "30px" }}>
        ...
        {kayitlar.length === 0 ? (
          ...
          ogunler.map((ogun) => {
            const ogunKayitlari = kayitlar.filter((item) => item.ogun === ogun.ad);
            const ogunToplam = ogunKayitlari.reduce((sum, item) => sum + Number(item.kalori), 0);
